...
Note: 
the model architecture, forward pass, loss, channel simulation and training process are the SAME between
Centralized DeepSC and Federated DeepSC.

sentence -> semantic encoder -> channel encoder -> noisy channel -> channel decoder -> semantic decoder -> sentence
(x,n) -> DeepSC (x,n)
"""
...
class DeepSC(nn.Module):
    """
    takes a sentence as a sequence of token ID
    embeds token IDs into word vectors
    uses self-attention to generate semantic representation
    outputs a semantic representation vector
    """
